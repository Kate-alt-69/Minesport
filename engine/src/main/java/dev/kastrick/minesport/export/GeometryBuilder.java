...
public class GeometryBuilder {
    ...
    private void rotateCorners(float[][] corners,BlockModel.Rotation rot){
        float ox=rot.origin[0]/16f,oy=rot.origin[1]/16f,oz=rot.origin[2]/16f;double rad=Math.toRadians(rot.angle);float cos=(float)Math.cos(rad),sin=(float)Math.sin(rad);
        for(float[] c:corners){
    ...
    private void applyBlockstateRotation(float[][] corners,int xRot,int yRot){
        ...
        for(float[] c:corners){float dx=c[0]-cx,dy=c[1]-cy,dz=c[2]-cz;if(yRot!=0){float cos=(float)Math.cos(yr),sin=(float)Math.sin(yr);float nx=dx*cos-dz*sin;dz=dx*sin+dz*cos;dx=nx;}if(xRot!=0){float cos=(float)Math.cos(xr),sin=(float)Math.sin(xr);float ny=dy*cos-dz*sin;dz=dy*sin+dz*cos;dy=ny;}c[0]=cx+dx;c[1]=cy+dy;c[2]=cz+dz;}
    }
    ...
    private static String rotateDirection(String dir,int xDeg,int yDeg){int[] v=DIR_VECTORS.get(dir);if(v==null)return dir;double dx=v[0],dy=v[1],dz=v[2];if(yDeg!=0){double r=Math.toRadians(yDeg),cos=Math.cos(r),sin=Math.sin(r),nx=dx*cos-dz*sin,nz=dx*sin+dz*cos;dx=nx;dz=nz;}if(xDeg!=0){double r=Math.toRadians(xDeg),cos=Math.cos(r),sin=Math.sin(r),ny=dy*cos-dz*sin,nz=dy*sin+dz*cos;dy=ny;dz=nz;}double ax=Math.abs(dx),ay=Math.abs(dy),az=Math.abs(dz);if(ax>=ay&&ax>=az)return dx>0?"east":"west";if(ay>=ax&&ay>=az)return dy>0?"up":"down";return dz>0?"south":"north";}
    private boolean isFaceOccluded(BlockData block,String worldDirection){int[] d=DIR_VECTORS.get(worldDirection);if(d==null)return false;BlockData n=occlusionIndex.get(SpatialKey.of(block.x+d[0],block.y+d[1],block.z+d[2]));if(n==null||n.isAir())return false;String opposite=OPPOSITE_DIR.get(worldDirection);Set<String> faces=cullableFacesCache.computeIfAbsent(cullCacheKey(n),k->computeCullableFaces(n));return faces.contains(opposite);}
    private Set<String> computeCullableFaces(BlockData b){Set<String> result=new HashSet<>();BlockState bs=resolvers.resolveBlockState(b.blockId);if(bs==null)return result;for(BlockState.ModelApplication app:bs.resolve(b.properties,b.x,b.y,b.z)){BlockModel model=resolvers.resolveModel(app.modelPath);if(model==null)continue;for(BlockModel.Element el:model.elements)for(BlockModel.Face face:el.faces.values())if(face.cullface!=null)result.add(rotateDirection(face.cullface,app.x,app.y));}return result;}
