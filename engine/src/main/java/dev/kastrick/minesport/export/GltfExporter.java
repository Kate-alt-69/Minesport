...
/** glTF 2.0 exporter with explicit per-vertex Minecraft UVs and compound objects. */
public class GltfExporter {
    ...
    public ObjExporter.ExportStats export(List<BlockData> blocks,GeometryBuilder builder,File outputFile,ObjExporter.ExportMode mode,boolean optimize,ObjExporter.ProgressCallback progress)throws IOException{
        ...
        for(var ge:groups.entrySet()){
            ...
            for(BlockData b:ge.getValue())for(Quad q:builder.buildBlock(b)){byTex.computeIfAbsent(q.texturePath(),k->new ArrayList<>()).add(q);quadCount++;}
            if(byTex.isEmpty())continue;
            JsonArray primitives=new JsonArray();
            for(var te:byTex.entrySet()){
                PrimitiveResult pr=buildPrimitive(te.getValue(),te.getKey(),center,optimize);
                if(pr==null)continue;
                primitives.add(pr.primitive());vertexCount+=pr.vertexCount();
            }
            if(primitives.size()==0)continue;
            JsonObject mesh=new JsonObject();mesh.addProperty("name",name.replace(':','_').replace('/','_'));mesh.add("primitives",primitives);meshes.add(mesh);
            JsonObject node=new JsonObject();
            String nodeName=mode==ObjExporter.ExportMode.ALL_MERGED?"Minesport_Export":name.replace(':','_').replace('/','_');
            if(nodeName.isBlank())nodeName="Minesport_Group_"+(mergedGroupIndex++);
            node.addProperty("name",nodeName);node.addProperty("mesh",meshes.size()-1);nodes.add(node);
        }
        ...
        JsonObject scene=new JsonObject();scene.addProperty("name","Minesport Export");JsonArray sceneNodes=new JsonArray();for(int i=0;i<nodes.size();i++)sceneNodes.add(i);scene.add("nodes",sceneNodes);JsonArray scenes=new JsonArray();scenes.add(scene);root.add("scenes",scenes);
    ...
    private PrimitiveResult buildPrimitive(List<Quad> quads,String texture,float[] center,boolean weld)throws IOException{
        ...
        for(Quad q:quads){float[][] vs=q.verts(),us=q.vertexUVs();int[] local=new int[4];
            for(int i=0;i<4;i++){
                float x=vs[i][0]-center[0],y=vs[i][1]-center[1],z=vs[i][2]-center[2];float[] n=q.normal();float[] t=us[i];
                String key=String.format(Locale.ROOT,"%.6f|%.6f|%.6f|%.4f|%.4f|%.4f|%.6f|%.6f",x,y,z,n[0],n[1],n[2],t[0],1f-t[1]);
                Integer existing=weld?map.get(key):null;
                if(existing!=null){local[i]=existing;continue;}
                int id=pos.size();local[i]=id;pos.add(new float[]{x,y,z});norm.add(new float[]{n[0],n[1],n[2]});uv.add(new float[]{t[0],1f-t[1]});if(weld)map.put(key,id);
            }
            idx.add(local[0]);idx.add(local[1]);idx.add(local[2]);idx.add(local[0]);idx.add(local[2]);idx.add(local[3]);
        }
        if(pos.isEmpty())return null;
        JsonObject prim=new JsonObject();JsonObject attrs=new JsonObject();int posAcc=writeVec3(pos,true),normAcc=writeVec3(norm,false),uvAcc=writeVec2(uv),idxAcc=writeIndices(idx);attrs.addProperty("POSITION",posAcc);attrs.addProperty("NORMAL",normAcc);attrs.addProperty("TEXCOORD_0",uvAcc);prim.add("attributes",attrs);prim.addProperty("indices",idxAcc);prim.addProperty("material",getMaterial(texture));prim.addProperty("mode",4);return new PrimitiveResult(prim,pos.size());
    }

    private int getMaterial(String texture)throws IOException{
        Integer existing=materialMap.get(texture);if(existing!=null)return existing;
        int texIndex=getTexture(texture);JsonObject pbr=new JsonObject();JsonObject base=new JsonObject();base.addProperty("index",texIndex);pbr.add("baseColorTexture",base);pbr.addProperty("metallicFactor",0);pbr.addProperty("roughnessFactor",1);JsonObject mat=new JsonObject();mat.addProperty("name",texture);mat.add("pbrMetallicRoughness",pbr);BufferedImage img=resolvers.resolveTexture(texture);if(img!=null&&hasAlpha(img))mat.addProperty("alphaMode","BLEND");materials.add(mat);int idx=materials.size()-1;materialMap.put(texture,idx);return idx;
    }

    private int getTexture(String texture)throws IOException{
        Integer existing=textureMap.get(texture);if(existing!=null)return existing;BufferedImage img=resolvers.resolveTexture(texture);if(img==null){img=new BufferedImage(16,16,BufferedImage.TYPE_INT_ARGB);for(int y=0;y<16;y++)for(int x=0;x<16;x++)img.setRGB(x,y,0xffff00ff);}
    ...
    private void writeBytes(byte[] bytes){bin.writeBytes(bytes);}
    private void pad4(){while((bin.size()&3)!=0)bin.write(0);}
    private static JsonArray toArray(List<JsonObject> list){JsonArray a=new JsonArray();for(JsonObject o:list)a.add(o);return a;}
}
