...
pub use legacy::{
    CaptureNotice, EPHEMERAL_ADDRESS, SNAPSHOT_SCHEMA, mods_fingerprint_filtered, snapshot_path,
};
...
impl StreamWriter {
    ...
    fn finish(mut self) -> Result<PathBuf> {
        ...
        // Prepare the matching readiness receipt before touching the last-good
        ...
        install_registry_snapshot(&self.temporary_path, &self.final_path, &staged_receipt)?;
        self.committed = true;
    ...
    if let Err(error) = install_result {
        ...
        if had_registry && registry_backup.exists() {
            if let Err(restore) = fs::rename(&registry_backup, final_path) {
                rollback_errors.push(format!("restore {}: {restore}", final_path.display()));
            }
        }
        if had_receipt && receipt_backup.exists() {
            if let Err(restore) = fs::rename(&receipt_backup, &ready_path) {
                rollback_errors.push(format!("restore {}: {restore}", ready_path.display()));
            }
        }
...
mod tests {
    ...
    fn failed_snapshot_replacement_restores_last_good_registry_and_receipt() {
        ...
        let final_path =
            write_empty_snapshot_for_test(&cache, "1.21.10", "transaction-test").unwrap();
        let ready_path = ready_receipt_path(&final_path);
        let old_registry = fs::read(&final_path).unwrap();
        let old_receipt = fs::read(&ready_path).unwrap();

        let staged_registry = final_path.parent().unwrap().join("new-registry.tmp");
        fs::write(&staged_registry, b"replacement-registry").unwrap();
        let missing_receipt = final_path.parent().unwrap().join("missing-ready.tmp");

        let error =
            install_registry_snapshot(&staged_registry, &final_path, &missing_receipt).unwrap_err();
