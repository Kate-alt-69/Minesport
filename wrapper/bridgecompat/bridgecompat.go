...
func Ensure(version string, progress ProgressFunc) (string, error) {
	...
	// Preparation rewrites a version-specific Gradle workspace. Serialize it so
	// repeated UI callbacks cannot delete or patch the same files concurrently.
	ensureMu.Lock()
	...
	builtJar, err := runGradleBuild(prepared.Workspace, javaHome, progress)
	if err != nil {
		return "", err
	}
...
func applyOperation(workspace string, manifest Manifest, operation PatchOperation, variables map[string]string) error {
	...
	case "regex_replace":
		...
		if !rx.Match(data) {
			return fmt.Errorf("expected regex %q was not found in %s", pattern, file)
		}
	...
	case "overlay":
		...
		target, err := workspacePath(workspace, expand(operation.Target))
		if err != nil {
			return err
		}
	...
	case "module":
		data, err := downloadPinnedModule(expand(operation.URL), expand(operation.SHA256))
		if err != nil {
			return fmt.Errorf("module %s: %w", operation.Name, err)
		}
		target, err := workspacePath(workspace, projectRelative(expand(operation.Target)))
		if err != nil {
			return err
		}
		if err := os.MkdirAll(filepath.Dir(target), 0o755); err != nil {
	...
	case "delete":
		...
		file, err := workspacePath(workspace, expand(target))
		if err != nil {
			return err
		}
...
func replaceAt(file string, line, column int, from, to string) error {
	if line < 1 || column < 1 || from == "" {
		return fmt.Errorf("rename_at requires 1-based line/column and non-empty from text")
	}
	data, err := os.ReadFile(file)
	if err != nil {
		return err
	...
	for current := 1; current < line; current++ {
		...
		if index < 0 {
			return fmt.Errorf("line %d does not exist in %s", line, file)
		}
		lineStart += index + 1
	}
	start := lineStart + column - 1
	if start < lineStart || start+len(from) > len(text) {
		return fmt.Errorf("line %d column %d is outside %s", line, column, file)
	}
	lineEnd := strings.IndexByte(text[lineStart:], '\n')
	if lineEnd < 0 {
		lineEnd = len(text) - lineStart
	} else if strings.HasSuffix(text[lineStart:lineStart+lineEnd], "\r") {
		lineEnd--
	}
	if start+len(from) > lineStart+lineEnd {
		return fmt.Errorf("rename_at text crosses line boundary in %s", file)
	}
	if text[start:start+len(from)] != from {
		return fmt.Errorf("stale rename_at in %s:%d:%d: expected %q, found %q", file, line, column, from, text[start:minInt(len(text), start+len(from))])
	}
...
func downloadPinnedModule(endpoint, expectedSHA string) ([]byte, error) {
	...
	if err != nil || parsed.Scheme != "https" || parsed.Host == "" {
		return nil, fmt.Errorf("module URL must use HTTPS")
	}
	if !sha256Pattern.MatchString(expectedSHA) {
		return nil, fmt.Errorf("module requires a 64-character SHA-256")
	}
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return nil, fmt.Errorf("HTTP %d downloading module", resp.StatusCode)
	}
	data, err := io.ReadAll(io.LimitReader(resp.Body, maxModuleBytes+1))
	if err != nil {
		return nil, err
	}
	if len(data) > maxModuleBytes {
		return nil, fmt.Errorf("module exceeds %d byte limit", maxModuleBytes)
	}
	actual := sha256.Sum256(data)
	if !strings.EqualFold(hex.EncodeToString(actual[:]), expectedSHA) {
		return nil, fmt.Errorf("module SHA-256 mismatch")
	}
	if !utf8.Valid(data) {
		return nil, fmt.Errorf("module is not valid UTF-8 text")
	}
	return data, nil
}
...
func replaceInFile(file, from, to string, optional bool) error {
	...
	if !strings.Contains(text, from) {
		...
		return fmt.Errorf("expected text %q was not found in %s", from, file)
	}
...
func resolveFabricLoader(version string) (string, error) {
	data, err := httpGet("https://meta.fabricmc.net/v2/versions/loader/" + url.PathEscape(version))
	if err != nil {
		return "", err
	}
	var response []struct {
	...
	if len(response) > 0 && response[0].Loader.Version != "" {
		return response[0].Loader.Version, nil
	}
...
func resolveFabricAPI(version string) (string, error) {
	...
	for _, candidate := range metadata.Versions {
		if strings.HasSuffix(candidate, "+"+version) {
			selected = candidate
		}
	}
	if selected == "" {
		return "", fmt.Errorf("Fabric API does not currently publish a build for Minecraft %s", version)
	}
	return selected, nil
}
...
func supportRoot() string {
	...
	if runtime.GOOS == "linux" {
		...
		if info, err := os.Stat(filepath.Join(system, "manifest.json")); err == nil && !info.IsDir() {
			return system
		}
	}
...
func report(progress ProgressFunc, percent int, stage, detail string) {
	if progress != nil {
		progress(Progress{Percent: percent, Stage: stage, Detail: detail})
	}
}
...
func runGradleBuild(workspace, javaHome string, progress ProgressFunc) (string, error) {
	...
	for _, line := range strings.Split(output.String(), "\n") {
		...
		if line != "" {
			report(progress, 68, "Compiling…", line)
		}
	}
	if err != nil {
		return "", fmt.Errorf("Gradle bridge compilation failed: %w\n%s", err, tailLines(output.String(), 40))
	}
	...
	for _, candidate := range matches {
		if !strings.Contains(strings.ToLower(filepath.Base(candidate)), "sources") {
			return candidate, nil
		}
	}
