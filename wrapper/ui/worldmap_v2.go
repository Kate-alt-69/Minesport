...
// WorldMapV2 is the modern map interaction surface. It intentionally keeps
// the coordinate math independent from the sidebar so view navigation cannot
// corrupt the selection state.
...
func (m *WorldMapV2) LoadHeightmap(img *image.RGBA, minX, minZ, maxX, maxZ int) {
	m.imgLock.Lock(); m.img = img; m.minX, m.minZ, m.maxX, m.maxZ = minX, minZ, maxX, maxZ; m.imgLock.Unlock()
	m.FitToWindow()
}
...
func (m *WorldMapV2) SetBubbleRadius(x,z int) { m.bubbleRX=x; m.bubbleRZ=z; m.Refresh() }
...
func (m *WorldMapV2) FitToWindow() {
	...
	m.zoom=math.Max(0.05, math.Min(zx,zy)*0.94)
...
func (m *WorldMapV2) screenToWorld(sx,sy float64)(int,int){
	w:=float64(m.maxX-m.minX+1); h:=float64(m.maxZ-m.minZ+1)
	if w<=0||h<=0{return 0,0}
	rx:=(sx-m.offsetX)/(w*m.zoom); rz:=(sy-m.offsetY)/(h*m.zoom)
	return m.minX+int(math.Floor(rx*w)), m.minZ+int(math.Floor(rz*h))
}
...
func (m *WorldMapV2) blockRect(x0,z0,x1,z1 int)(float64,float64,float64,float64){
	a,b:=m.worldToScreen(x0,z0); c,d:=m.worldToScreen(x1+1,z1+1); return a,b,c,d
}

// clampView is the important fix for the old "234564 x 234567" problem:
// zoom and pan are bounded together, so repeated wheel events cannot turn
// the map transform into enormous floating-point coordinates.
func (m *WorldMapV2) clampView(){
	if m.zoom<0.05 {m.zoom=0.05}; if m.zoom>64 {m.zoom=64}
	m.imgLock.RLock(); img:=m.img; m.imgLock.RUnlock(); if img==nil{return}
	w:=float64(img.Bounds().Dx())*m.zoom; h:=float64(img.Bounds().Dy())*m.zoom
	pad:=float64(math.Max(32,float64(m.size.Width)*0.1))
...
func (m *WorldMapV2) MouseDown(ev *desktop.MouseEvent){
	...
	if ev.Button==desktop.MouseButtonMiddle { // Blender-style MMB pan in 2D
		m.panning=true; m.panStartX=x; m.panStartY=y; m.panOffsetX=m.offsetX; m.panOffsetY=m.offsetY; return
	}
	if ev.Button==desktop.MouseButtonSecondary && m.bubble {
		wx,wz:=m.screenToWorld(x,y); m.bubbleCenter=[2]int{wx,wz}; m.hasBubble=true; if m.OnCenterPicked!=nil{m.OnCenterPicked(wx,wz)}; m.Refresh(); return
	}
	if ev.Button==desktop.MouseButtonPrimary {
		wx,wz:=m.screenToWorld(x,y); m.dragging=true; m.selA=[2]int{wx,wz}; m.selB=m.selA; m.Refresh()
	}
}
func (m *WorldMapV2) MouseUp(ev *desktop.MouseEvent){
	...
	m.dragging=false
	if m.selA!=m.selB { minX,minZ:=m.selA[0],m.selA[1]; maxX,maxZ:=m.selB[0],m.selB[1]; if minX>maxX{minX,maxX=maxX,minX}; if minZ>maxZ{minZ,maxZ=maxZ,minZ}; if m.OnSelectionChanged!=nil{m.OnSelectionChanged(minX,minZ,maxX,maxZ)} }
	m.Refresh()
}
func (m *WorldMapV2) MouseMoved(ev *desktop.MouseEvent){
	...
	wx,wz:=m.screenToWorld(x,y); if m.OnCursorMoved!=nil{m.OnCursorMoved(wx,wz)}
	if wx!=m.hover[0]||wz!=m.hover[1]||!m.hasHover{m.hover=[2]int{wx,wz};m.hasHover=true;m.Refresh()}
...
func (m *WorldMapV2) Scrolled(ev *fyne.ScrollEvent){
	// Clamp each wheel event instead of allowing large accumulated DY values
	// to multiply the transform into absurd offsets.
	dy:=ev.Scrolled.DY; if dy>3{dy=3}; if dy< -3{dy=-3}
	factor:=math.Pow(1.08,dy)
	old:=m.zoom; next:=old*factor; if next<0.05{next=0.05};if next>64{next=64}
	factor=next/old
	cx,cy:=float64(ev.Position.X),float64(ev.Position.Y)
	m.offsetX=cx-(cx-m.offsetX)*factor; m.offsetY=cy-(cy-m.offsetY)*factor; m.zoom=next
...
type worldMapV2Renderer struct{m *WorldMapV2;bg,img,sel,hover,bubble *canvas.Rectangle}
func(r *worldMapV2Renderer)MinSize()fyne.Size{return r.m.MinSize()};func(r *worldMapV2Renderer)Destroy(){}
