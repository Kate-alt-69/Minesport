...
type Request struct {
    ...
    WorldPath string `json:"worldPath,omitempty"`; OutputPath string `json:"outputPath,omitempty"`; Format string `json:"format,omitempty"`
    MinX int `json:"minX,omitempty"`; MinY int `json:"minY,omitempty"`; MinZ int `json:"minZ,omitempty"`; MaxX int `json:"maxX,omitempty"`; MaxY int `json:"maxY,omitempty"`; MaxZ int `json:"maxZ,omitempty"`
    ExportMode string `json:"exportMode,omitempty"`; Options map[string]string `json:"options,omitempty"`
...
type Response struct {
    ...
    File string `json:"file,omitempty"`; Count int `json:"count,omitempty"`; BlockCount int `json:"blockCount,omitempty"`; QuadCount int `json:"quadCount,omitempty"`; VertexCount int `json:"vertexCount,omitempty"`
}
...
type ExportParams struct {WorldPath string;OutputPath string;Format string;ExportMode string;MinX,MinY,MinZ int;MaxX,MaxY,MaxZ int;Options map[string]string;CenterX,CenterY,CenterZ *int;RadiusX,RadiusY,RadiusZ *int}
func(e *Engine)Export(p ExportParams)error{return e.Send(Request{Command:"export",WorldPath:p.WorldPath,OutputPath:p.OutputPath,Format:p.Format,MinX:p.MinX,MinY:p.MinY,MinZ:p.MinZ,MaxX:p.MaxX,MaxY:p.MaxY,MaxZ:p.MaxZ,ExportMode:p.ExportMode,Options:p.Options,CenterX:p.CenterX,CenterY:p.CenterY,CenterZ:p.CenterZ,RadiusX:p.RadiusX,RadiusY:p.RadiusY,RadiusZ:p.RadiusZ})}
...
func(e *Engine)ListBlocks(p ListBlocksParams)(string,int,error){resp,err:=e.SendCommand(map[string]interface{}{"command":"listBlocks","worldPath":p.WorldPath,"minX":p.MinX,"minY":p.MinY,"minZ":p.MinZ,"maxX":p.MaxX,"maxY":p.MaxY,"maxZ":p.MaxZ,"centerX":p.CenterX,"centerY":p.CenterY,"centerZ":p.CenterZ,"radiusX":p.RadiusX,"radiusY":p.RadiusY,"radiusZ":p.RadiusZ});if err!=nil{return "",0,err};if resp.Type=="error"{return "",0,fmt.Errorf("%s",resp.Message)};return resp.File,resp.Count,nil}
func(e *Engine)Ping()error{return e.Send(Request{Command:"ping"})}
func(e *Engine)Stop(){e.mu.Lock();defer e.mu.Unlock();if e.stdin!=nil{_=json.NewEncoder(e.stdin).Encode(Request{Command:"quit"});_=e.stdin.Close()};if e.cmd!=nil&&e.cmd.Process!=nil{_=e.cmd.Process.Kill()};e.ready=false}
