...
type outEvent struct {
    ...
    Min, Max [3]int  `json:"min,omitempty"`
...
func Run(blocksPath string) error {
    ...
    updateSelection := func() {
        if point1 == nil || point2 == nil { return }
        p1, p2 := *point1, *point2
        ...
        setHighlight(boxed)
        window.SetTitle(fmt.Sprintf("Minesport — 3D · %s selected · E+scroll resize · LMB confirm", formatCount(len(boxed))))
    }
    confirmSelection := func() {
        ...
        mx := [3]int{max(p1[0], p2[0]), max(p1[1], p2[1]), max(p1[2], p2[2])}
        boxed := blocksInBox(blocks, mn, mx)
        coords := make([][3]int, len(boxed))
        for i, b := range boxed { coords[i] = [3]int{b.X, b.Y, b.Z} }
        emit(outEvent{Type: "selection", Blocks: coords, Min: mn, Max: mx, Count: len(coords)})
        setHighlight(boxed)
        resizeMode = false
        window.SetTitle(fmt.Sprintf("Minesport — 3D · %s selected · confirmed", formatCount(len(boxed))))
    }
    adjustSelectionByLook := func(step int) {
        if point2 == nil { return }
        d := cam.Forward()
        ax, ay, az := math.Abs(float64(d.X)), math.Abs(float64(d.Y)), math.Abs(float64(d.Z))
        p := *point2
        if ax >= ay && ax >= az { if d.X >= 0 { p[0] += step } else { p[0] -= step } } else if ay >= ax && ay >= az { if d.Y >= 0 { p[1] += step } else { p[1] -= step } } else { if d.Z >= 0 { p[2] += step } else { p[2] -= step } }
        point2 = &p
        updateSelection()
    }
    ...
    window.SetMouseButtonCallback(func(w *glfw.Window, button glfw.MouseButton, action glfw.Action, mods glfw.ModifierKey) {
        ...
        if button == glfw.MouseButtonLeft && action == glfw.Press {
            ...
            if !ok { return }
            if point1 == nil { p := pos; point1 = &p; emit(outEvent{Type: "point1Set", X: p[0], Y: p[1], Z: p[2]}) } else if resizeMode { confirmSelection() } else { p := pos; point2 = &p; updateSelection() }
        }
    ...
    window.SetScrollCallback(func(w *glfw.Window, xoff, yoff float64) {
        if resizeMode && point1 != nil && point2 != nil {
            if yoff > 0 { adjustSelectionByLook(1) } else if yoff < 0 { adjustSelectionByLook(-1) }
            return
        }
        Dolly(cam, float32(yoff))
    })
    window.SetKeyCallback(func(w *glfw.Window, key glfw.Key, scancode int, action glfw.Action, mods glfw.ModifierKey) {
        if action == glfw.Press && key == glfw.KeyE { resizeMode = point1 != nil && point2 != nil; updateSelection() }
        if action == glfw.Press && key == glfw.KeyEscape { point1 = nil; point2 = nil; resizeMode = false; setHighlight(nil); setTitleStats() }
    })
        ...
        for {
            ...
            case cmd, ok := <-cmdCh:
                ...
                case "floodFill":
                    ...
                    for i, b := range r { coords[i] = [3]int{b.X, b.Y, b.Z} }
...
func blocksInBox(all []Block, mn, mx [3]int) []Block {
    ...
    out := make([]Block, 0)
    for _, b := range all { if b.X >= lx && b.X <= hx && b.Y >= ly && b.Y <= hy && b.Z >= lz && b.Z <= hz { out = append(out, b) } }
    return out
}
...
func formatCount(n int) string {
    ...
    s := fmt.Sprintf("%d", n)
    o := ""
    for i, c := range s { if i > 0 && (len(s)-i)%3 == 0 { o += "," }; o += string(c) }
    return o
}
