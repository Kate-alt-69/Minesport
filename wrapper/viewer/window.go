...
func Run(blocksPath string) error {
	...
	if len(blocks) == 0 { return fmt.Errorf("no blocks") }
	index := BuildIndex(blocks)
	if err = glfw.Init(); err != nil { return err }
	...
	setHighlight := func(h []Block) {
		if highlightMesh != nil { highlightMesh.Destroy(); highlightMesh = nil }
		if len(h) > 0 { highlightMesh = buildHighlightMesh(h) }
	}
	...
	selectionBounds := func() ([3]int, [3]int, bool) {
		if point1 == nil || point2 == nil { return [3]int{}, [3]int{}, false }
		p1, p2 := *point1, *point2
	...
	updateSelection := func() {
		mn, mx, ok := selectionBounds(); if !ok { return }
	...
	adjustSelectionByLook := func(step int) {
		if point2 == nil || step == 0 { return }
		d := cam.Forward()
		ax, ay, az := math.Abs(float64(d.X)), math.Abs(float64(d.Y)), math.Abs(float64(d.Z))
		p := *point2
		if ax >= ay && ax >= az {
			if d.X >= 0 { p[0] += step } else { p[0] -= step }
		} else if ay >= ax && ay >= az {
			if d.Y >= 0 { p[1] += step } else { p[1] -= step }
		} else {
			if d.Z >= 0 { p[2] += step } else { p[2] -= step }
		}
		point2 = &p
		updateSelection()
	}
	...
	window.SetMouseButtonCallback(func(w *glfw.Window, button glfw.MouseButton, action glfw.Action, mods glfw.ModifierKey) {
		if button == glfw.MouseButtonMiddle {
			...
			first = true
			if middleHeld { w.SetInputMode(glfw.CursorMode, glfw.CursorDisabled) } else { w.SetInputMode(glfw.CursorMode, glfw.CursorNormal) }
			return
		}
		if action != glfw.Press { return }
		if button == glfw.MouseButtonLeft {
			if point1 != nil && point2 != nil { confirmSelection(); return }
			_, pos, ok := Raycast(index, cam.Position, cam.Forward(), 200); if !ok { return }
			p := pos; point1 = &p; point2 = nil; setHighlight(nil)
			emit(outEvent{Type: "point1Set", X: p[0], Y: p[1], Z: p[2]})
			window.SetTitle("Minesport — 3D · point A set · RMB chooses point B")
			return
		}
		if button == glfw.MouseButtonRight {
			if point1 == nil { return }
			_, pos, ok := Raycast(index, cam.Position, cam.Forward(), 200); if !ok { return }
			p := pos; point2 = &p; updateSelection()
		}
	})
	window.SetCursorPosCallback(func(w *glfw.Window, x, y float64) {
		if !middleHeld { return }
		if first { lastX, lastY = x, y; first = false; return }
		dx, dy := float32(x-lastX), float32(y-lastY); lastX, lastY = x, y
		OrbitPan(cam, dx, dy, shiftPan)
	})
	window.SetScrollCallback(func(w *glfw.Window, xoff, yoff float64) {
		if yoff == 0 { return }
		if resizeHeld && point1 != nil && point2 != nil {
			if yoff > 0 { adjustSelectionByLook(1) } else { adjustSelectionByLook(-1) }
			return
		}
	...
	window.SetKeyCallback(func(w *glfw.Window, key glfw.Key, scancode int, action glfw.Action, mods glfw.ModifierKey) {
		if key == glfw.KeyE {
			...
			if resizeHeld && point1 != nil && point2 != nil { updateSelection() }
			return
		}
		if action == glfw.Press && key == glfw.KeyEscape {
			point1 = nil; point2 = nil; resizeHeld = false; setHighlight(nil); setTitleStats()
		}
	})

	cmdCh := make(chan inCommand, 16)
	go func() {
		s := bufio.NewScanner(os.Stdin); s.Buffer(make([]byte, 0, 64*1024), 8*1024*1024)
		for s.Scan() { var c inCommand; if json.Unmarshal([]byte(s.Text()), &c) == nil { cmdCh <- c } }
		close(cmdCh)
	}()
	setTitleStats(); emit(outEvent{Type: "ready", Count: len(blocks)})
	...
	drain:
		for {
			...
			case cmd, ok := <-cmdCh:
				...
				switch cmd.Command {
				case "floodFill":
					r := FloodFillJoined(index, [3]int{cmd.X, cmd.Y, cmd.Z}, cmd.Power); setHighlight(r)
					coords := make([][3]int, len(r)); for i, b := range r { coords[i] = [3]int{b.X, b.Y, b.Z} }
					emit(outEvent{Type: "selection", Blocks: coords, Count: len(coords)})
				case "highlightBox": setHighlight(blocksInBox(blocks, cmd.Min, cmd.Max))
				case "clearHighlight": setHighlight(nil); setTitleStats()
				case "quit": quit = true
				}
			default: break drain
			}
		...
		w, h := window.GetSize(); if h == 0 { h = 1 }
		gl.Viewport(0, 0, int32(w), int32(h)); gl.ClearColor(0.53, 0.71, 0.90, 1); gl.Clear(gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT)
		proj := Perspective(float32(70*math.Pi/180), float32(w)/float32(h), 0.1, 2000); view := cam.ViewMatrix(); model := Identity()
		gl.UseProgram(program); gl.UniformMatrix4fv(modelLoc, 1, false, &model[0]); gl.UniformMatrix4fv(viewLoc, 1, false, &view[0]); gl.UniformMatrix4fv(projLoc, 1, false, &proj[0]); mesh.Draw(posLoc, colorLoc, normalLoc)
		if highlightMesh != nil { gl.UseProgram(highlightProgram); gl.UniformMatrix4fv(hModelLoc, 1, false, &model[0]); gl.UniformMatrix4fv(hViewLoc, 1, false, &view[0]); gl.UniformMatrix4fv(hProjLoc, 1, false, &proj[0]); gl.Uniform3f(hColorUniformLoc, 0.25, 1, 0.45); highlightMesh.Draw(hPosLoc, hColorLoc, hNormalLoc) }
		window.SwapBuffers(); glfw.PollEvents()
	}
	if highlightMesh != nil { highlightMesh.Destroy() }
	return nil
}

func buildHighlightMesh(blocks []Block) *Mesh { copied := make([]Block, len(blocks)); copy(copied, blocks); return buildMeshInflated(copied, 0.03) }
func blocksInBox(all []Block, mn, mx [3]int) []Block { lx, hx := min(mn[0], mx[0]), max(mn[0], mx[0]); ly, hy := min(mn[1], mx[1]), max(mn[1], mx[1]); lz, hz := min(mn[2], mx[2]), max(mn[2], mx[2]); out := make([]Block, 0); for _, b := range all { if b.X >= lx && b.X <= hx && b.Y >= ly && b.Y <= hy && b.Z >= lz && b.Z <= hz { out = append(out, b) } }; return out }
func min(a, b int) int { if a < b { return a }; return b }
func max(a, b int) int { if a > b { return a }; return b }
func formatCount(n int) string { if n < 0 { n = 0 }; s := fmt.Sprintf("%d", n); o := ""; for i, c := range s { if i > 0 && (len(s)-i)%3 == 0 { o += "," }; o += string(c) }; return o }
