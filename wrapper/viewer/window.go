...
func Run(blocksPath string) error {
    ...
    window.SetMouseButtonCallback(func(w *glfw.Window, button glfw.MouseButton, action glfw.Action, mods glfw.ModifierKey) {
        if button == glfw.MouseButtonMiddle {
            ...
            shiftPan = middleHeld && (mods&glfw.ModifierShift) != 0
            first = true
    ...
    window.SetKeyCallback(func(w *glfw.Window, key glfw.Key, scancode int, action glfw.Action, mods glfw.ModifierKey) {
        if action == glfw.Press && key == glfw.KeyE {
            resizeMode = point1 != nil && point2 != nil
            updateSelection()
        }
    ...
    go func() {
        ...
        for s.Scan() {
            var c inCommand
            if json.Unmarshal([]byte(s.Text()), &c) == nil { cmdCh <- c }
        }
        close(cmdCh)
    }()
...
func blocksInBox(all []Block, mn, mx [3]int) []Block {
    ...
    out := make([]Block, 0)
    for _, b := range all {
        if b.X >= lx && b.X <= hx && b.Y >= ly && b.Y <= hy && b.Z >= lz && b.Z <= hz { out = append(out, b) }
    }
    return out
}
