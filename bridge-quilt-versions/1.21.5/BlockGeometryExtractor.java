...
public class BlockGeometryExtractor {
    ...
    public static List<BlockVariant> extractBlock(Block block, Minecraft client) {
        ...
        for (BlockState state : block.getStateDefinition().getPossibleStates()) {
            ...
            state.getValues().forEach((prop, val) -> props.put(prop.getName(), val.toString()));

            List<BakedQuadData> quads = extractQuads(model, stableSeed(block, state), convertedQuads);
        ...
        for (Direction dir : DIRECTIONS) {
            ...
            for (BlockModelPart part : parts) {
                ...
                for (BakedQuad quad : baked) {
                    BakedQuadData[] cache = convertedQuads.computeIfAbsent(quad, ignored -> new BakedQuadData[7]);
    ...
    private static BakedQuadData convertQuad(BakedQuad quad, Direction dir) {
        try {
            ...
            if (vertexData == null || vertexData.length < 24 || vertexData.length % 4 != 0) return null;
            ...
            String textureId = sprite != null ? sprite.contents().name().toString() : "missing";
            int faceIdx = dir != null ? dir.ordinal() : -1;
            return new BakedQuadData(vertices, textureId, faceIdx, quad.shade(), quad.tintIndex());
        } catch (Exception e) {
            return null;
        }
    }
